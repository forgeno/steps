...
test.requestHooks(logger)("attempt to rate a sidewalk 3 times within an hour and fail on the fourth on", async (t) => {
	...
	await t.expect(notifications.text.textContent).contains("Cannot rate same sidewalk within 1 hour.");
});
...
test.requestHooks(logger)("attempt to rate a sidewalk 3 times within an hour and fail on the fourth on", async (t) => {
	...
	await t.expect(notifications.text.textContent).contains("Too many Sidewalks rated within 30 seconds.");
});